...
export const EVENT_LABELS = {
  ...
  guidance: { ko: '가이던스', en: 'Guidance' },
  contract: { ko: '수주', en: 'Contract' },
  regulation: { ko: '규제', en: 'Regulation' },
  lawsuit: { ko: '소송', en: 'Lawsuit' },
  recall: { ko: '리콜/사고', en: 'Recall/Incident' },
 ...
 * @property {string} source - 출처
 * @property {string} url - 원문 링크
 * @property {string} title - 기사 제목
 * @property {string} publishedAt - ISO 날짜
 * @property {string} [snippet] - 요약
 */

/**
 * @typedef {Object} Event
 * @property {string} id - 고유 ID
 * @property {'kr'|'us'} market - 시장
 * @property {string} ticker - 종목코드
 * @property {string} startedAt - 사건 시작 시각 (ISO)
 * @property {string} updatedAt - 마지막 업데이트 (ISO)
 * @property {string} type - 사건 라벨
 * @property {string[]} summary2 - 요약 2줄
 * @property {string} why - 왜 중요한지 1줄
 * @property {Article[]} links - 근거 링크
 */
...
export const SAMPLE_EVENTS = [
  // 삼성전자
  {
    ...
    type: 'guidance',
    summary2: [
      '삼성전자, 2026년 HBM 생산능력 40% 확대 계획 발표.',
      '엔비디아向 HBM3E 공급 계약 체결 임박 소식.'
    ],
  ...
  },
  {
    ...
    type: 'earnings',
    summary2: [
      '삼성전자 2025년 4분기 영업이익 12.3조원, 전년比 +45%.',
      'HBM·AI 서버 DRAM 호조로 DS부문 흑자 전환 확정.'
    ],
  ...
  // SK하이닉스
  {
    ...
    type: 'contract',
    summary2: [
      'SK하이닉스, 엔비디아와 2026년 HBM3E 독점 공급 계약 체결.',
      '계약 규모 약 15조원, 연간 공급량 전년 대비 2배 증가.'
    ],
  ...
  // 테슬라
  {
    ...
    type: 'recall',
    summary2: [
      '테슬라, 사이버트럭 12만대 자발적 리콜 발표.',
      '스티어링 시스템 소프트웨어 결함, OTA 업데이트로 해결 예정.'
    ],
  ...
  },
  {
    ...
    type: 'earnings',
    summary2: [
      '테슬라 4분기 인도량 51만대, 시장 예상 48만대 상회.',
      '연간 인도량 195만대로 역대 최고 기록 경신.'
    ],
  ...
  // 엔비디아
  {
    ...
    type: 'guidance',
    summary2: [
      '엔비디아, 블랙웰 GPU 2분기 대량 양산 돌입 확인.',
      'MS·아마존·구글 등 빅테크 주문량 전분기 대비 3배 증가.'
    ],
  ...
  // 애플
  {
    ...
    type: 'guidance',
    summary2: [
      '애플, 아이폰17 프로에 자체 AI 칩 탑재 확정.',
      '온디바이스 AI 성능 40% 향상, 클라우드 의존도 대폭 감소.'
    ],
  ...
  // 현대차
  {
    ...
    type: 'contract',
    summary2: [
      '현대차, 미국 조지아 공장 EV 생산량 연 40만대로 확대.',
      'IRA 보조금 적용 차종 확대, 연간 2.5조원 세액공제 혜택.'
    ],
  ...
  // 네이버
  {
    ...
    type: 'other',
    summary2: [
      '네이버, AI 검색 큐(Cue:) 월간 사용자 2,000만 돌파.',
      '기존 검색 대비 체류시간 35% 증가, 광고 수익성 개선 기대.'
    ],
  ...
  // 메타
  {
    ...
    type: 'earnings',
    summary2: [
      '메타, 쓰레드 MAU 5억 돌파로 트위터(X) 추월.',
      '광고 수익화 본격 시작, 2026년 매출 기여 50억 달러 전망.'
    ],
  ...
  // KB금융
  {
    ...
    type: 'earnings',
    summary2: [
      'KB금융, 2025년 배당금 4,800원 확정, 전년比 +20%.',
      '주주환원율 53% 달성, 금융지주 중 최고 수준.'
    ],
  ...
  // JP모건
  {
    ...
    type: 'macro',
    summary2: [
      'JP모건, 2026년 연준 금리 인하 3회 전망 유지.',
      '상반기 동결 후 하반기 75bp 인하 예상.'
    ],
  ...
  // 추가 샘플들 - 다양한 라벨 커버
  {
    ...
    type: 'regulation',
    summary2: [
      '셀트리온, FDA 바이오시밀러 신약 승인 획득.',
      '미국 시장 진출 확대, 연간 매출 1.5조원 추가 기대.'
    ],
  ...
  },
  {
    ...
    type: 'lawsuit',
    summary2: [
      '구글, 미국 법무부 반독점 소송 1심 패소.',
      '검색엔진 시장 지배력 남용 인정, 구제책 협의 예정.'
    ],
 ...
 * 최근 24시간 이벤트
 */
export function getEventsLast24h(market, ticker) {
  const cutoff = new Date(Date.now() - 24 * 60 * 60 * 1000);
  return getEventsByTicker(market, ticker).filter(e => new Date(e.startedAt) > cutoff);
}

/**
 * 최근 7일 이벤트
 */
export function getEventsLast7d(market, ticker) {
 ...
 * 오늘의 주요 사건 (전체 종목)
 */
export function getTodayTopEvents(limit = 5) {
  const cutoff = new Date(Date.now() - 24 * 60 * 60 * 1000);
 ...
 * 라벨별 이벤트 필터링
 */
export function getEventsByLabel(type) {
  return SAMPLE_EVENTS.filter(e => e.type === type)
    .sort((a, b) => new Date(b.startedAt) - new Date(a.startedAt));
}
